...
function getApplyList(){
	$.ajax({
		url : "/booth/boothApplyAudit/queryAllBoothApplyList",
		type : "post",
		dataType : "json",
		async : false,
		cache : false,
...
function audit(boothApplyId){
	var auditFirstState=$('input[name="'+boothApplyId+'"]:checked').val();
	var audit_operation = "audit_operation" + boothApplyId;
	var auditFirstOpinion = $('#'+audit_operation+'').val();
	$.ajax({
		url : '/booth/boothApplyAudit/auditByTeacher',
		async : false,
		cache : false,
		type : "post",
		dataType : 'json',
		data : {
			auditFirstState : auditFirstState,
			auditFirstOpinion : auditFirstOpinion,
